...
{
  //if we initialized the heap and paging
  if(kheap)
  {
    void *addr = kalloc(sz, (uint8_t)align, kheap);
    if(phys)
    {
      page_t *page = get_page((uint32_t)addr, 0, kernel_directory);
      //~ page_t *page = get_page((uint32_t)addr, 0, current_directory == 0 ? kernel_directory : current_directory);
...
{
  uint32_t *out;
  out = (uint32_t*)kmalloc_int(new_sz, 1, 0);
  memcpy(out, ptr, old_sz);
  kfree(ptr);
  return (uint32_t)out;
}
...
{
  uint32_t *out;
  out = (uint32_t*)kmalloc_int(new_sz, 1, phys);
  memcpy(out, ptr, old_sz);
  kfree(ptr);
  return (uint32_t)out;
}
...
{
  uint32_t *out;
  out = (uint32_t*)kmalloc_int(new_sz, 0, phys);
  memcpy(out, ptr, old_sz);
  kfree(ptr);
  return (uint32_t)out;
}
...
void *kalloc(uint32_t size, uint8_t page_align, heap_t *heap)
{
  ...
  if(orig_hole_size - new_size > 0)
  {
    ...
    //footer_t *hole_footer = (footer_t *)((uint8_t*)hole_header + orig_hole_size - new_size - sizeof(footer_t) );
...
{
  //the uint32_t before the start of the actual data holds the size of the whole alloc, including headers and footers
  return *((uint32_t*)alloc - 1) -  (sizeof(header_t) + sizeof(footer_t));
}
